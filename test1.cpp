...
void simpanPengguna() {
    try {
        ...
        if (!f.is_open()) {
            throw runtime_error("Gagal membuka file untuk menyimpan data pengguna.");
        }
        f << jumlahPengguna << "\n";
...
void loadPengguna() {
    try {
        ...
        cout << "[!] Gagal load data pengguna: " << e.what() << "\n";
    }
}
...
void simpanRiwayatLogin(string namaUser, string role, string status) {
    try {
        ...
        if (!f.is_open()) {
            throw runtime_error("Gagal membuka file riwayat login.");
        }
...
void tampilRiwayatLogin() {
    try {
        ...
        if (!f.is_open()) {
            throw runtime_error("File riwayat login tidak ditemukan.");
        }
...
// --- DIUBAH: tambah validasi & throw untuk input kosong ---
...
// --- DIUBAH: tambah validasi & throw untuk input harga ---
...
int pilihNomor(string aksi) {
    ...
    try {
        ...
        int nomor = bacaAngka(pilih); // bisa throw invalid_argument
        if (nomor < 1 || nomor > no)
            throw out_of_range("Nomor di luar jangkauan! Pilih antara 1 sampai " + angkaKeStr(no) + ".");
...
void tambahProduk() {
    cout << "\n--- Tambah Produk Baru ---\n";
    if (totalProduk >= MAKS_PRODUK) {
        cout << "Data produk udah penuh nih!\n";
        return;
    }
    ...
    try {
        ...
        p.harga.satuan   = bacaHarga(hargaStr); // bisa throw
        p.harga.matauang = "IDR";
        p.stok.jumlah    = bacaAngka(stokStr);  // bisa throw
        p.stok.satuan    = "pcs";
        p.aktif          = 1;
        ...
        cout << "[!] Input tidak valid: " << e.what() << "\n";
        cout << "    Produk gagal ditambahkan.\n";
    } catch (const out_of_range& e) {
        cout << "[!] Nilai di luar batas: " << e.what() << "\n";
        cout << "    Produk gagal ditambahkan.\n";
    }
}
...
void editProduk() {
    ...
    try {
        ...
        if (hargaBaru.size() > 0) {
            try {
                p.harga.satuan = bacaHarga(hargaBaru);
            } catch (const exception& e) {
                cout << "[!] Harga tidak diubah: " << e.what() << "\n";
            }
        }
        ...
        if (stokBaru.size() > 0) {
            try {
                p.stok.jumlah = bacaAngka(stokBaru);
            } catch (const exception& e) {
                cout << "[!] Stok tidak diubah: " << e.what() << "\n";
            }
        }
        ...
        cout << "[!] Terjadi error saat edit: " << e.what() << "\n";
    }
}

void nonaktifkanProduk(int *statusAktif, string namaProduk) {
    if (statusAktif == nullptr)
        throw runtime_error("Pointer status produk bernilai NULL!");
    *statusAktif = 0;
...
void hapusProduk() {
    ...
    try {
        ...
        if (konfirm == "y" || konfirm == "Y")
            nonaktifkanProduk(&daftarProduk[idx].aktif, daftarProduk[idx].nama); // bisa throw
        else
            cout << "Penghapusan dibatalkan.\n";

    } catch (const runtime_error& e) {
        cout << "[!] Error hapus produk: " << e.what() << "\n";
    }
}
...
void cariNama() {
    ...
    try {
        ...
        else throw invalid_argument("Pilihan jenis tidak valid. Masukkan angka 1-8.");
...
void cariHarga() {
    ...
    try {
        ...
        double target = bacaHarga(inputHarga); // bisa throw invalid_argument / out_of_range
...
void kurangiStok(int *stokPtr, int jumlah) {
    if (stokPtr == nullptr)
        throw runtime_error("Pointer stok produk bernilai NULL!");
    if (jumlah <= 0)
        throw invalid_argument("Jumlah pengurangan stok harus lebih dari 0.");
    if (*stokPtr < jumlah)
        throw out_of_range("Stok tidak mencukupi untuk dikurangi sejumlah " + angkaKeStr(jumlah) + ".");
    *stokPtr -= jumlah;
}
...
int tampilProdukMerk(string merk, int peta[], int modeSort) {
    ...
    if      (modeSort == 1) cout << "  [ Diurutkan: Nama Z -> A ]\n";
    else if (modeSort == 2) cout << "  [ Diurutkan: Harga Termurah -> Termahal ]\n";
    else if (modeSort == 3) cout << "  [ Diurutkan: Stok Sedikit -> Terbanyak ]\n";
    else                    cout << "  [ Urutan: Default ]\n";
...
void tambahKeKeranjangDariMerk(ItemKeranjang keranjang[], int &jmlKeranjang,
    ...
    try {
        if (jmlKeranjang >= MAKS_KERANJANG)
            throw length_error("Keranjang udah penuh! Maksimal " + angkaKeStr(MAKS_KERANJANG) + " item.");
        ...
        int nomorProduk = bacaAngka(pilihanProduk); // bisa throw invalid_argument
        if (nomorProduk < 1 || nomorProduk > no)
            throw out_of_range("Nomor produk di luar jangkauan.");

        int    idx = peta[nomorProduk - 1];
        Produk &p  = daftarProduk[idx];
        ...
        if (stokTersedia <= 0)
            throw out_of_range("Stok '" + p.nama + "' udah habis dimasukin ke keranjang!");

        cout << "Mau beli berapa " << p.nama << "? (stok tersedia: " << stokTersedia << "): ";
        getline(cin, pilihanJumlah);

        int jml = bacaAngka(pilihanJumlah); // bisa throw invalid_argument
        if (jml <= 0)
            throw invalid_argument("Jumlah harus lebih dari 0.");
        if (jml > stokTersedia)
            throw out_of_range("Jumlah melebihi stok tersedia (" + angkaKeStr(stokTersedia) + ")!");
...
void prosesCheckout(ItemKeranjang keranjang[], int &jmlKeranjang, string namaUser) {
    ...
    try {
        ...
        if (pilihanMetode == "1") {
            ...
            jumlahBayar = bacaHarga(inputBayar); // bisa throw invalid_argument
            if (jumlahBayar < grandTotal)
                throw runtime_error("Uang kurang " + rupiahFormat(grandTotal - jumlahBayar) + "! Pembayaran gagal.");
            ...
            throw invalid_argument("Metode pembayaran tidak valid.");
        }
        ...
        for (int i = 0; i < jmlKeranjang; i++)
            kurangiStok(&daftarProduk[keranjang[i].idxProduk].stok.jumlah, keranjang[i].jumlah); // bisa throw
    ...
    while (true) {
        ...
        if (pilihan == "7") {
            ...
            try {
                ...
                int idxHapus = bacaAngka(hapusStr) - 1; // bisa throw invalid_argument
                if (idxHapus < 0 || idxHapus >= jmlKeranjang)
                    throw out_of_range("Nomor item di luar jangkauan.");
...
void doRegister() {
    if (jumlahPengguna >= MAKS_PENGGUNA) {
        cout << "Slot pengguna udah penuh nih!\n";
        return;
    }
    ...
    try {
        ...
        for (int i = 0; i < jumlahPengguna; i++) {
            if (dataPengguna[i].nama == nama)
                throw runtime_error("Nama '" + nama + "' udah dipake, coba nama lain ya!");
        }
